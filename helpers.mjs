...
export async function createNewDeployment(node, forkingChainId) {
  ...
  let mnemonicWallet = ethers.Wallet.fromPhrase(node.mnemonic);

  const newEnvVariables = `
ETHERSCAN_API_KEY=verifyContract
SCRIPT_RPC_URL=${node.rpcUrl}
VERIFIER_URL=${node.verificationUrl}
SCRIPT_PRIVATE_KEY=${mnemonicWallet.privateKey}
`;
  fs.writeFileSync(envFilePath, newEnvVariables);
